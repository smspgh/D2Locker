...
test('d2l-custom-symbols symbols map', async () => {
  setupi18n();
  ...
  for (const { codepoint, source } of symbolData) {
    ...
    if (!def) {
      throw new Error(
        `No definition exists for ${codepoint}: ${JSON.stringify(source)}. You may need to update the d2l-custom-symbols repo.`,
      );
    }
  }
  ...
  for (const [key, value] of Object.entries(conversionTable)) {
    ...
    if (!value.plaintext || value.plaintext.length < 3) {
      throw new Error(
        `Conversion table entry ${key} has no plaintext. You may need to update the d2l-custom-symbols repo.`,
      );
    }
    if (!value.unicode) {
      throw new Error(
        `Conversion table entry ${key} has no unicode. You may need to update the d2l-custom-symbols repo.`,
      );
    }
  }
  ...
  for (const value of symbolsMap) {
    if (!value.name) {
      throw new Error(
        `Symbol ${JSON.stringify(value)} has no name. You may need to update the d2l-custom-symbols repo.`,
      );
    }
    if (!value.fullName) {
      throw new Error(
        `Symbol ${JSON.stringify(value)} has no fullName. You may need to update the d2l-custom-symbols repo.`,
      );
    }
  }
});
