...
export interface DimApiState {
  ...
  /*
   * D2L API profile data, per account. The key is `${platformMembershipId}-d${destinyVersion}`.
   */
  profiles: {
    [accountKey: string]: {
      ...
      /** This allows us to get just the items that changed from the D2L API instead of the whole deal. */
      sync?: string;
    };
  };
...
function getInitialApiPermissionSetting() {
  const setting = localStorage.getItem('d2l-api-enabled');
  if (setting === null) {
    return null;
...
/**
 * Global D2L platform settings from the D2L API.
 */
  ...
  switch (action.type) {
    ...
    case getType(actions.setApiPermissionGranted): {
      ...
      return apiPermissionGranted
        ...
        : // If we're disabling d2l sync, unset profile loaded and clear the update queue
          {
            ...state,
            apiPermissionGranted,
            profileLoaded: false,
...
function migrateSettings(state: DimApiState) {
  ...
  for (const destinyVersion in migratedSearches) {
    ...
    if (searches && Array.isArray(searches)) {
      migratedSearches[destinyVersion as keyof typeof migratedSearches] = searches.map(
        (search: Search) => ({
...
function changeSetting<V extends keyof Settings>(state: DimApiState, prop: V, value: Settings[V]) {
  ...
  return produce(state, (draft) => {
    ...
    const update: ProfileUpdateWithRollback = {
      ...
      // Settings updates are global, but we still need to assign them to a profile
      // so they can be sent to the API. We use the currently selected account.
      // If there's no account selected, these updates will be dropped.
      // TODO: This means settings updates might not get sent if you change them before logging in.
      platformMembershipId: Object.keys(state.profiles).length
        ? parseProfileKey(Object.keys(state.profiles)[0])[0]
        : undefined,
      destinyVersion: Object.keys(state.profiles).length
        ? parseProfileKey(Object.keys(state.profiles)[0])[1]
        : undefined,
    };
...
/**
 * This prepares the update queue to be flushed to the D2L API. It first
...
function prepareUpdateQueue(state: DimApiState) {
  return produce(state, (draft) => {
    ...
    for (const update of draft.updateQueue) {
      ...
      // because D2L API update can only work one profile at a time.
...
function applyFinishedUpdatesToQueue(state: DimApiState, results: ProfileUpdateResult[]) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (!(result.status === 'Success' || result.status === 'NotFound')) {
      ...
      errorLog('d2l sync', update.action, result.status, message, result.message, update);
      reportException('d2l sync', new Error('invalid d2l api update'), {
      ...
      infoLog('d2l sync', update.action, result.status, message, update);
    }
  }
...
function applyUpdateLocally(draft: Draft<DimApiState>, update: ProfileUpdateWithRollback) {
  switch (update.action) {
    ...
    case 'search': {
      ...
      const lastUsage = $D2L_FLAVOR === 'test' ? 1000 : Date.now();
