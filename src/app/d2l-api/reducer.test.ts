...
describe('d2l api reducer', () => {
  const cases: {
    ...
    },
    {
      ...
      actions: [setItemTag({ itemId: '1234', tag: 'keep' })],
      checkState: (state) => {
        expect(state.profiles[currentAccountKey].tags['1234'].tag).toBe('keep');
      },
      expectedQueue: [
        {
          action: 'tag',
    ...
    },
    {
      ...
      actions: [
        setItemTag({ itemId: '1234', tag: 'keep' }),
    ...
    },
    {
      ...
      actions: [setItemHashTag({ itemHash: 1234, tag: 'keep' })],
      checkState: (state) => {
        expect(state.itemHashTags[1234].tag).toBe('keep');
      },
      expectedQueue: [
        {
    ...
    },
    {
      ...
      actions: [
        setItemHashTag({ itemHash: 1234, tag: 'keep' }),
    ...
    },
    {
      ...
      checkState: (state) => {
        // FIXME maybe delete this outright? It'll be cleaned up the next time D2L loads the remote profile anyway...
        const search = state.searches[2][0];
    ...
    it(name, () => {
      ...
      const updatedState = [...actions, prepareToFlushUpdates()].reduce((s, action) => {
        if (Array.isArray(action)) {
          return d2lApi(s, action[0], action[1]);
        }
        return d2lApi(s, action, currentAccount);
      }, setupState);
...
describe('prepareToFlushUpdates', () => {
  ...
  for (const {
    ...
    it(name, () => {
      ...
      ].reduce((s, action) => d2lApi(s, action, currentAccount), initialState);
...
describe('finishedUpdates', () => {
  it('can mark success', () => {
    let state = d2lApi(initialState, setSettingAction('showNewItems', true));
    state = d2lApi(state, setItemTag({ itemId: '1234', tag: 'keep' }), currentAccount);
    state = d2lApi(state, prepareToFlushUpdates());

    const updatedState = d2lApi(
      state,
