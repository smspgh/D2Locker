...
try {
  syncChannel = new BroadcastChannel('dim-sync');
} catch (e) {
...
const installObservers = once((dispatch: ThunkDispatch<RootState, undefined, AnyAction>) => {
  ...
  // Also check for DIM API updates when the page becomes visible
  ...
  // Set up a periodic check for DIM API updates (settings, loadouts, etc.)
