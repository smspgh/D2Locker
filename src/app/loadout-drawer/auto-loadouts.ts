...
export function itemLevelingLoadout(allItems: DimItem[], store: DimStore): Loadout {
  ...
  const bestItemFn = (item: DimItem) => {
    ...
    value += ItemRarityMap[item.rarity] * 10;
...
export function itemMoveLoadout(items: DimItem[], store: DimStore): Loadout {
  ...
  const groupedItems = Object.groupBy(items, (i) => i.bucket.hash);
  const itemsByType: { [bucketHash: number]: DimItem[] } = {};
  ...
  const finalItems = Object.values(itemsByType)
    .flat()
    .filter((i): i is DimItem => i !== undefined) // Filter out undefined items
...
function addUpStackables(items: DimItem[]) {
  return Object.values(Object.groupBy(items, (t) => t.hash)).flatMap((itemsInGroup) => {
    ...
    if (firstItem.maxStackSize > 1) {
      const item = { ...firstItem, amount: sumBy(itemsInGroup, (i) => i.amount) };
      return [item];
    } else {
      return itemsInGroup;
    }
