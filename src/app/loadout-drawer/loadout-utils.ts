...
/**
 * Determines the light-level weight for a D1 item based on its type.
 */
/**
 * Determines the light-level weight for a D1 item based on its type.
 */
function getD1ItemWeight(item: DimItem, weights: LookupTable<BucketSortType, number>): number {
  // Class Armor is always "General"
  if (item.bucket.hash === BucketHashes.ClassArmor) {
    // Provide a fallback of 1 in case 'General' isn't in the table
    // or if noUncheckedIndexedAccess is on.
    return weights.General ?? 1;
  }

  // Use the item's bucket sort property if it exists
  if (item.bucket.sort) {
    // Provide a fallback for invalid sort keys.
    return weights[item.bucket.sort] ?? 1;
  }

  // Default weight if no sort property (should be rare)
  return 1;
}
...
export function getLight(store: DimStore, items: DimItem[]): number {
  ...
  if (store.destinyVersion === 2) {
    ...
    const itemWeightTable: LookupTable<BucketSortType, number> = {
      Weapons: 6,
      Armor: 5,
      General: 4,
    };
    ...
    const weightedItems = items
      ...
      .map((item) => ({
        power: item.power || 0,
        weight: getD1ItemWeight(item, itemWeightTable),
      }));

    const totalWeight = sumBy(weightedItems, (i) => i.weight);
    if (totalWeight === 0) {
      return 0;
    }

    const totalPower = sumBy(weightedItems, (i) => i.power * i.weight);
    const exactLight = totalPower / totalWeight;
...
export function optimalItemSet(
  ...
  const classUnrestricted = Object.values(anyClassBestItemByBucket)
    ...
    .sort(compareByIndex(gearSlotOrder, (i) => i.bucket.hash));
  ...
  for (const overlappingItems of overlaps.values()) {
    ...
    for (const item of overlappingItems) {
      ...
      for (const otherItem of otherItems) {
        ...
        const bucketItems = thisClassItemsByBucket[otherItem.bucket.hash];
        if (!bucketItems) {
          // No items in this bucket for this class
          optionValid = false;
          break;
        }
        const nonExotics = bucketItems.filter((i) => !i.equippingLabel);
        if (nonExotics.length) {
          const bestNonExotic = maxBy(nonExotics, bestItemFn);
          if (bestNonExotic) {
            option[otherItem.bucket.hash] = bestNonExotic;
          } else {
            optionValid = false;
          }
        } else {
  ...
  const equippable = Object.values(equippableBestItemByBucket)
    .filter((item): item is DimItem => item !== undefined)
    .sort(compareByIndex(gearSlotOrder, (i) => i.bucket.hash));
