...
export default function registerServiceWorker() {
  ...
  // Check for updates when the app becomes visible again (PWA is reopened)
  document.addEventListener('visibilitychange', async () => {
    if (!document.hidden) {
      try {
        ...
        errorLog(TAG, 'Failed to check version on visibility change', e);
      }
    }
  });
  ...
  window.addEventListener('load', () => {
    navigator.serviceWorker
      ...
      .then((registration) => {
        // Force immediate update check when service worker is registered
        registration.update();
        ...
        updateServiceWorker = async () => {
          ...
          if (registration.waiting) {
            ...
            // Sentry has been disabled - this code is no longer needed
            // Previously this would disable Sentry error logging for older versions
