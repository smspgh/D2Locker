...
export default function AllWishlistRolls({
  ...
  const wishlistRolls = useSelector(wishListRollsForItemHashSelector(item.hash)) ?? []; // Provide default empty array
  ...
  return (
    <>
      {Object.entries(groupedWishlistRolls).map(([notes, rolls]) => {
        // Ensure rolls is an array, even if Object.groupBy returns undefined for a key
        const actualRolls = rolls || [];
        const consolidatedRolls = consolidateRollsForOneWeapon(defs, item, actualRolls);
