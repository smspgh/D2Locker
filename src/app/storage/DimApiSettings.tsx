...
export default function DimApiSettings() {
  ...
  return (
    <section className={styles.storage} id="storage">
      ...
      {STORAGE_SETTINGS_VISIBILITY.D2LSync && (
      ...
      {(STORAGE_SETTINGS_VISIBILITY.importBackup || STORAGE_SETTINGS_VISIBILITY.exportAPIProfile) && (
        <div className={settingClass}>
          <ImportExport
            ...
            showImportBackup={STORAGE_SETTINGS_VISIBILITY.importBackup}
            showExportAPIProfile={STORAGE_SETTINGS_VISIBILITY.exportAPIProfile}
          />
        </div>
      )}
    </section>
...
function showBackupDownloadedNotification() {
  showNotification({
    type: 'success',
    title: t('Storage.D2LSyncEnabled'),
