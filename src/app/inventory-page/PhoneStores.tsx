...
function StoresInventory({
  ...
  return (
    <>
      {/* Always render StoreStats at the top, below the character header */}
      <div className="store-cell" style={{ paddingBottom: 8 }}>
        <StoreStats store={store} />
      </div>

      {/* Render Engrams and Postmaster side-by-side */}
      <div className={styles.engramsAndPostmaster}>
        {buckets.byCategory.Engrams && buckets.byCategory.Engrams.length > 0 && (
          <div className={styles.engramsSection}>
            {buckets.byCategory.Engrams.map(renderBucket)}
          </div>
        )}
        {buckets.byCategory.Postmaster && buckets.byCategory.Postmaster.length > 0 && (
          <div className={styles.postmasterSection}>
            {buckets.byCategory.Postmaster.map(renderBucket)}
          </div>
        )}
      </div>
